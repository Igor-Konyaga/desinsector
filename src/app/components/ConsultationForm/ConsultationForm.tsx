...
import { fetchTelegtamMessage } from '@/app/services/telegremMessage';
...
export const ConsultationForm = () => {
  ...
  async function submitForm(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const { data } = await fetchTelegtamMessage(message);
      setStatusOk(data.ok);
    } catch (error) {
      setError(error);
    } finally {
  ...
  return (
    <section id="consultationForm" className={styles.section}>
      <form onSubmit={submitForm} className={styles.form}>
        <h2 className={styles.title}>Безкоштовна консультація</h2>
